...
-- Versión: MySQL 5.5.62 Compatible
-- Fecha: 2025-01-08
...
-- =====================================================
-- PASO 1: CREAR TABLAS DE AUTENTICACIÓN (MySQL 5.5 Compatible)
-- =====================================================
...
-- Tabla de usuarios del sistema (MySQL 5.5 Compatible)
CREATE TABLE IF NOT EXISTS users (
    ...
    FOREIGN KEY (service_id) REFERENCES servicios(id_servicio) ON DELETE SET NULL,
    FOREIGN KEY (employee_id) REFERENCES empleados(id_empleado) ON DELETE SET NULL,
...
-- Tabla de sesiones para auditoría (MySQL 5.5 Compatible)
...
INSERT IGNORE INTO user_roles (id, name, display_name, level, description) VALUES
...
-- Servicios
('manage_all_services', 'Gestionar Todos los Servicios', 'services', 'manage', 'Crear, editar y eliminar cualquier servicio del hospital'),
('manage_own_service', 'Gestionar Propio Servicio', 'services', 'manage_own', 'Gestionar solo el servicio asignado al usuario'),
('view_all_services', 'Ver Todos los Servicios', 'services', 'view_all', 'Ver información de todos los servicios del hospital'),
('view_own_service', 'Ver Propio Servicio', 'services', 'view_own', 'Ver solo el servicio asignado al usuario'),

-- Empleados
('manage_all_employees', 'Gestionar Todos los Empleados', 'employees', 'manage_all', 'CRUD completo de todos los empleados'),
('manage_service_employees', 'Gestionar Empleados del Servicio', 'employees', 'manage_service', 'Gestionar empleados del propio servicio únicamente'),
('view_all_employees', 'Ver Todos los Empleados', 'employees', 'view_all', 'Ver información de todos los empleados del hospital'),
('view_service_employees', 'Ver Empleados del Servicio', 'employees', 'view_service', 'Ver empleados del propio servicio únicamente'),
('view_own_profile', 'Ver Propio Perfil', 'employees', 'view_own', 'Ver y editar información personal del usuario'),

-- Horarios
('manage_all_schedules', 'Gestionar Todos los Horarios', 'schedules', 'manage_all', 'CRUD completo de horarios de todos los servicios'),
('manage_service_schedules', 'Gestionar Horarios del Servicio', 'schedules', 'manage_service', 'Gestionar horarios del propio servicio únicamente'),
('view_all_schedules', 'Ver Todos los Horarios', 'schedules', 'view_all', 'Ver horarios de todos los servicios del hospital'),
('view_service_schedules', 'Ver Horarios del Servicio', 'schedules', 'view_service', 'Ver horarios del propio servicio únicamente'),
('view_own_schedule', 'Ver Propio Horario', 'schedules', 'view_own', 'Ver únicamente el horario personal del usuario'),

-- Intercambios de turnos
('approve_shift_changes', 'Aprobar Cambios de Turno', 'shifts', 'approve', 'Aprobar o rechazar solicitudes de cambio de turno'),
('request_shift_changes', 'Solicitar Cambios de Turno', 'shifts', 'request', 'Solicitar cambios en los turnos asignados'),
('request_shift_exchange', 'Solicitar Intercambio', 'shifts', 'exchange', 'Solicitar intercambio de turnos con otros empleados'),

-- Reportes
('view_all_reports', 'Ver Todos los Reportes', 'reports', 'view_all', 'Acceso completo a todos los reportes del sistema'),
('view_service_reports', 'Ver Reportes del Servicio', 'reports', 'view_service', 'Ver reportes únicamente del propio servicio'),

-- Configuración
('manage_holidays', 'Gestionar Feriados', 'holidays', 'manage', 'Crear, editar y eliminar días feriados del sistema'),
('manage_users', 'Gestionar Usuarios', 'users', 'manage', 'CRUD completo de usuarios del sistema'),
('system_settings', 'Configuración del Sistema', 'system', 'settings', 'Acceso a configuraciones globales del sistema');
...
('admin_hospital', 'view_own_profile');
...
-- Crear usuario super administrador inicial (MySQL 5.5 Compatible)
...
SELECT 
    ...
    display_name,
    level,
    (SELECT COUNT(*) FROM role_permissions WHERE role_id = ur.id) as permissions_count
FROM user_roles ur
ORDER BY level;
...
-- =====================================================
-- NOTAS IMPORTANTES PARA MYSQL 5.5.62
-- =====================================================

/*
CAMBIOS ESPECÍFICOS PARA MYSQL 5.5:
- INDEX → KEY (sintaxis más antigua)
- BOOLEAN → TINYINT(1)
- UUID() no disponible → IDs manuales
- utf8mb4 → utf8
- ON UPDATE CASCADE simplificado
- Collation eliminada
